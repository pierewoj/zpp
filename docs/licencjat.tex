...
\chapter{Implemented binarization algorithms}
	In order to get binarized network from standard network we implemented two binarization algorithms proposed by XORNET paper. The main difference is that first one binarized only filters while second change both filters and activation to binary. 
    ...
    \section{Binarized weights}
	    ...
	    If we binarize the weights, we can use the fact that to compute convolution we do not need to perform multiplication. This results in roughly two times shorter computation time \cite{xnornet}.
